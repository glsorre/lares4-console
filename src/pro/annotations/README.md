...
The code in this directory implements row-pin and log-entry bookmarks
(with notes + export) for the Lares4 console.
...
- `types.ts` — `Bookmark` (re-exported from `src/core/log-store`)
- `ui/BookmarksPane.tsx` — shadcn pane for listing, noting, and exporting bookmarks

Imported via the `@pro/annotations` path alias.
