...
The code in this directory implements macro authoring, recording, and
playback for the Lares4 console.
...
- `types.ts` — `Macro`, `MacroStep`, `DEFAULT_STEP_DELAY_MS`
- `engine.ts` — `MacroEngine` playback class
- `ui/MacroEditorDialog.tsx` — shadcn dialog for creating and editing macros

Imported via the `@pro/macros` path alias.
